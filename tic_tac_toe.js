...
function winable() {
    ...
    for (let i = 0; i < 3; i++) {
        let counter = 0;
        for (let j = 0; j < 2; j++) {
            if (items[i * 3 + j].textContent === items[i * 3 + j + 1].textContent && items[i * 3 + j].textContent !== '') {
                counter++;
            }
        }
        if (counter === 2) {
            win = true;
            shownotification(!step ? 'Победили Х' : 'Победили О', 'win'); // Исправлено
            return;
        }
    }
}
...
function verticalWin() {
    ...
    for (let i = 0; i < 3; i++) {
        let counter = 0;
        for (let j = 0; j < 2; j++) {
            if (items[j * 3 + i].textContent === items[j * 3 + i + 3].textContent && items[j * 3 + i].textContent !== '') {
                counter++;
            }
        }
        if (counter === 2) {
